...
export function ActionNotice({ notice, onClose }: { notice: ActionNoticeState; onClose: () => void }) {
  ...
  return (
    <div className={`fixed right-5 top-5 z-[99999] flex w-[min(92vw,420px)] items-start gap-3 border p-4 shadow-2xl ${tone}`}>
      <Icon className="mt-0.5 h-5 w-5 shrink-0" />
      <div className="min-w-0 flex-1 text-sm font-semibold">{notice.message}</div>
      <button type="button" onClick={onClose} className="shrink-0 opacity-70 hover:opacity-100" aria-label="Close notice"><X className="h-4 w-4" /></button>
    </div>
  );
}
