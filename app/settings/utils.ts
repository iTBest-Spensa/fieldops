...
export function settingsToForm(settings: DbSettings): SettingsForm {
  return {
    ...
    defaultCustomerBillingRate: String(settings.default_customer_billing_rate ?? 0),
    defaultTechnicianPayRate: String(settings.default_technician_pay_rate ?? 0),
    minimumBillableMinutes: String(settings.minimum_billable_minutes ?? 0),
    travelBillingMode: settings.travel_billing_mode ?? "time",
    travelHourlyRate: settings.travel_hourly_rate === null || settings.travel_hourly_rate === undefined ? "" : String(settings.travel_hourly_rate),
    travelPerKmRate: String(settings.travel_per_km_rate ?? 0),
