...
function SingleLineTimeline({ track }: { track: Segment[] }) {
  ...
  return (
    <div className="relative min-h-[118px] overflow-hidden px-3 py-2">
      ...
      <div className="relative h-[100px]">
        ...
        {track.map((segment, index) => {
          ...
          const topLabel = segment.labelSide === "top";
          ...
          return (
            ...
            >
              <div className={`absolute left-0 right-0 top-[47px] h-[4px] ${colors.line}`} />
              ...
              >
                <div className={`truncate text-[9px] font-black uppercase ${colors.text}`}>
                  {colors.label} · {segment.id}
                </div>
                <div className="truncate text-[10px] font-semibold">{segment.title}</div>
                <div className="whitespace-nowrap text-[9px] text-muted-foreground">{segment.time}</div>
              </div>
            </div>
          );
        })}
