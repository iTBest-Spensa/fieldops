...
const technicians: Technician[] = [
  {
    ...
    role: "Network",
    status: "ON SITE",
    statusTone: "border-blue-500 bg-blue-500/10 text-blue-500",
  ...
  },
  {
    ...
    role: "Security",
    status: "WORKING",
    statusTone: "border-orange-500 bg-orange-500/10 text-orange-500",
...
function TrackHint({
  ...
  return (
    <div
      className="pointer-events-none absolute left-1/2 z-[80] w-60 -translate-x-1/2 border border-border bg-background p-3 text-foreground opacity-100 shadow-2xl"
      style={{ top: "78px" }}
    >
      ...
      <div className="relative h-[100px]">
        ...
        {items.map((item, index) => {
          if (item.type === "gap") {
            ...
            const key = `gap-${item.start}-${item.end}-${index}`;
            ...
            return (
              <button
                ...
                className="absolute top-0 z-30 h-full cursor-pointer focus:outline-none focus:ring-1 focus:ring-primary/50"
                style={{
          ...
          const key = `${segment.id}-${segment.time}-${index}`;
          const isOpen = openKey === key;
          ...
          return (
            <button
              ...
              className="absolute top-0 z-30 h-full cursor-pointer text-left focus:outline-none focus:ring-1 focus:ring-primary/50"
              style={{
                left: `${Math.max(0, left)}%`,
