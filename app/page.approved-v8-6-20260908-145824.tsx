...
function HoverHintPortal({ hint }: { hint: HoverHintData | null }) {
  ...
  return createPortal(
    <div
      className="pointer-events-none fixed z-[99999] w-64 border border-slate-300 bg-white p-3 text-slate-950 shadow-2xl dark:border-slate-700 dark:bg-slate-950 dark:text-white"
      style={{
        left: hint.left,
        top: hint.top,
    ...
    >
      ...
      <div className="text-[10px] font-black uppercase tracking-wider text-primary">{hint.status}</div>
...
function SingleLineTimeline({ track }: { track: Segment[] }) {
  ...
  function openHint(
    ...
    const useAbove = rect.bottom + 190 > window.innerHeight;
